...
방향을 가질 수 있다. (directed, undirected)

트리와 달리 사이클을 가질 수 있다.

### 표현 방식

그래프는 인접 행렬 방식과 인접 리스트의 방식으로 표현 가능하다

#### 인접 행렬

그래프를 2차원 배열로 만들어 노드의 연결 여부 또는 비용을 적는다.

배열로 구현되었기에 조회 시 O(1)로 가능하다.

노드 개수 * 노드 개수만큼의 2차원 배열이 필요하기에 공간이 낭비되고 모든 노드에 엣지 정보를 대입해야 한다.
...
인접 행렬보다 조회 속도가 느리고 구현이 비교적 어렵다.

### 검색

깊이 우선 검색(DFS), 넓이 우선 검색(BFS)가 있다.

#### DFS

그래프의 자식 노드를 우선으로 검색하는 방법

스택 자료구조를 활용한다,

## Hash

배열(Hash table)에 key를 Hash method를 사용하여 얻은 인덱스(Hash code)에 값을 저장하는 방식

배열을 사용하여 검색이 빠르다.

## Hash method

들어온 문자열(값)을 내부 연산을 통해 정수를 반환한다.

문자열의 길이는 무제한이지만 정수는 그렇지 않기에 다른 값이 들어와도 같은 값을 반환하는 경우가 생긴다.

이를 해쉬 충돌이라고 하며 해쉬 충돌을 줄이는 해쉬 메소드의 알고리즘이 중요하다.
